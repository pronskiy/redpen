...
- **A1.1 — Scaffold.** ✅ Deliverable met: `npm run tauri dev` boots an app with a hidden window and a menu-bar tray icon. Window is created hidden so it can later be converted to a panel *before* first show. Also set here: `ActivationPolicy::Accessory`, so there is no Dock icon, no app menu, and the process never becomes the active application — the premise Epic B rests on. Frontend is Vite + vanilla TS (HMR for the C1 card iteration, no framework weight in a HUD). Tray art was the stock Tauri icon; the menu-bar template image landed in C1.5.
...
| C1.5 | App icon + menu-bar template image | ✅ | replaces the stock Tauri art on both surfaces; `scripts/build-icons.sh` regenerates from SVG |
...
- **C1.1 — Card.** ✅ Borderless translucent card, `NSVisualEffectMaterial::Popover` so it follows the system light/dark appearance. Roman chose the direction: **inline annotation** for the fragment/rewrite pairing, **adaptive vibrancy** for the treatment.
  **Layout is chosen per fragment, not globally.** Measured against the corpus, 46% of rewrites change ≤2 words (inline is ideal), 33% change 3–4, and 21% restructure the sentence entirely — where an inline marker would highlight most of the line and explain nothing. So a word-level edit span picks inline or stacked per note. Two smaller calls: the card sets `user-select: none`, since a rewrite you can copy quietly undoes decision #1; and the mark is crimson `#b03a4a` against green `#0a7c3e` for the rewrite. *(This step originally called for amber over red, on the grounds that half of what redpen flags is grammatically correct English. That never reached code — the CSS has been crimson since the card was first built. Settled in favour of the shipped colour by decision #33.)*
...
- **C1.5 — Icon.** ✅ Deliverable: no stock Tauri art left in the build. **Two pieces, not one** — see decision #33. The bundle icon (`design/appicon.svg`) is the margin mark: a paragraph with one word struck in crimson and the fix hanging off the elbow, i.e. the card itself as a picture, and only the changed word is marked, per C1.2. The tile is a true superellipse `|x/412|^5 + |y/412|^5 = 1` at 824px on a 1024 canvas — both numbers measured off a real macOS system icon, matching Apple's silhouette to a 0.34px mean deviation at 256px, since Tauri's `icon` command applies no mask of its own.
  The menu-bar glyph (`design/tray-template.svg`) is a separate drawing, because a template image is pure black plus alpha and none of the colour survives. There the struck word becomes two thin bars whose gap *is* the pen stroke, with the correction solid and indented below. It is authored at 38×36 px on a whole-pixel grid: `tray-icon` forces the NSImage to 18pt tall, so that is an exact 2× on retina, and half-pixel coordinates render as grey mush at this size.
  Detail is aimed at 128px and up, where a Finder icon actually lives; below 64px it resolves to warm paper with a red and a green mark. Nothing is legible at 16px — no Mac icon is — and the job there is a distinctive colour and silhouette.
